...
function ChatWidgetContent({ fullScreen = false }) {
  ...
  useEffect(() => {
    if ((fullScreen || isOpen) && !isMinimized) {
      scrollMessagesToBottom()
    }
  }, [messages, isOpen, isMinimized, fullScreen])

  useEffect(() => {
    if ((fullScreen || isOpen) && !isMinimized) {
      inputRef.current?.focus()
      // Ensure the input is visible within the chat container without page jump
      scrollMessagesToBottom()
    }
  ...
  // If rendering as an embedded full-screen panel
  if (fullScreen) {
    return (
      <div className="relative z-20 w-full h-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-lg flex flex-col overflow-hidden overscroll-none">
        {/* Header */}
        <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-t-2xl">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="text-xs opacity-75">{isTyping ? 'Typing...' : 'Online'}</div>
            </div>
          </div>
        </div>

        {/* Messages */}
        <div ref={listRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-4 scroll-smooth overscroll-contain">
          {messages.map((m, idx) => (
  ...
  // Floating widget visibility can be disabled per-page by adding a body class
  const hideFloating = typeof document !== 'undefined' && document.body.classList.contains('hide-floating-chat')
          ...
          <div className={`bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl flex flex-col transition-all duration-300 ${
            ...
            {!isMinimized && (
              <>
                ...
                <div className="px-4 py-2 border-t border-gray-200 dark:border-gray-700">
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => setInput('Summarize the main points of this document')} className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors duration-200">Summarize</button>
                    <button onClick={() => setInput('What are the key insights from this document?')} className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors duration-200">Key Insights</button>
                    <button onClick={testConnection} className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors duration-200">Test API</button>
                    <button onClick={clearChat} className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors duration-200">Clear Chat</button>
                  </div>
                </div>
                <form onSubmit={sendMessage} className="p-4 border-t border-gray-200 dark:border-gray-700">
                  <div className="flex space-x-2">
                    <input ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about your documents..." disabled={isTyping} className="flex-1 px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 focus:ring-2 focus:ring-indigo-500 focus:border-transparent disabled:opacity-50" />
                    <button type="submit" disabled={!input.trim() || isTyping} className="p-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl hover:from-indigo-600 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105">
                      <Send className="w-4 h-4" />
                    </button>
                  </div>
                </form>
    ...
    <ErrorBoundary
      fallback={
        <div className="fixed bottom-6 right-6 z-50 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl p-4">
          <p className="text-red-600 dark:text-red-400">Something went wrong with the chat widget. Please refresh the page.</p>
          <button onClick={() => window.location.reload()} className="mt-2 px-3 py-1 text-sm bg-red-100 text-red-800 rounded-md hover:bg-red-200">Refresh</button>
        </div>
      }
    >
